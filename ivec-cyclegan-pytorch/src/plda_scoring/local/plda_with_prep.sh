...
if [ "$pre_processing" == "true" ]; then
  ...
  run.pl $prep_ivec_dir/log/plda.log \
    ivector-compute-plda ark:$prep_data_dir/spk2utt \
    "ark:ivector-subtract-global-mean scp:${prep_ivec_dir}/ivector.scp ark:- | transform-vec ${prep_ivec_dir}/transform.mat ark:- ark:- | ivector-normalize-length ark:- ark:- |" \
