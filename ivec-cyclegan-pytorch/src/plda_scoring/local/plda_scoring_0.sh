...
if [ "$use_existing_models" == "true" ]; then
  ...
  run.pl $plda_ivec_dir/log/plda.log \
    ...
    "ark:transform-vec ${plda_ivec_dir}/transform.mat ark:${plda_ivec_dir}/ivector.scp ark:- | ivector-normalize-length ark:- ark:- |" \
...
run.pl $scores_dir/log/plda_scoring.log \
  ivector-plda-scoring --normalize-length=true \
    ...
    "ark:ivector-mean ark:${enroll_data_dir}/spk2utt scp:${enroll_ivec_dir}/ivector.scp ark:- | ivector-subtract-global-mean ${plda_ivec_dir}/mean.vec scp:${enroll_ivec_dir}/spk_ivector.scp ark:- | transform-vec ${plda_ivec_dir}/transform.mat ark:- ark:- | ivector-normalize-length ark:- ark:- |" \
