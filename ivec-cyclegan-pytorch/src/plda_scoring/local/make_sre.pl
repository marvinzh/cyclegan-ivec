...
open(CS, "<$db_base/doc/MIXER_callstat.tbl") || die  "Could not open $db_base/doc/callstat.tbl";
#open(CI, "<$db_base/DISC1/doc/callinfo.tbl") || die  "Could not open $db_base/doc/callinfo.tbl";
...
while (<CS>) {
  ...
  if (/$wav/i ~~ @badAudio) {
    ...
    if (-e "$wav{$wav}") {
      $uttId = $spkr1 . "-". $gender1 ."-" . $wav ."-$spkch1";
      if (!$spk2gender{$spkr1}) {
        $spk2gender{$spkr1} = $gender1;
        print GNDR "$spkr1"," $gender1\n";
      }
      print WAV "$uttId"," sph2pipe -f wav -p -c $spkch2 $wav{$wav} |\n";
